...
<body>
	...
	<div id="content" class="hide">
		<form class="form-horizontal">
			...
			<div class="control-group">
				<label class="control-label"><s>*</s>所属字典：</label>
				<div class="controls">
					<input type="text" id="show" name="cnName" data-rules="{required:true}"/>					
					<input	type="hidden" id="hide" name="dict_id" />
				</div>
			</div>
		</form>
	</div>
	...
	<script type="text/javascript">
		BUI
				.use(
						['bui/picker','bui/grid', 'bui/data'],
						function(Picker,Grid, Data) {
							...
							var store = new Data.Store({
								url : '/ms/flow/dict/dictItemList',
								autoLoad : true, //自动加载数据								
								pageSize : 5,
								proxy : {
									method : 'POST', //更改为POST
									save : {
										addUrl : '/ms/flow/dict/save',//新增提交的URL
										removeUrl : '/ms/shiro/user/delete',//删除提交的URL
										updateUrl : '/ms/flow/dict/update'//编辑提交的URL
									},
									ajaxOptions : {
							...
							//
							var rowediting = new Grid.Plugins.RowEditing();
							var dictstore = new Data.Store({
								url : '/ms/flow/dict/dictList',
								autoLoad : true,
								pageSize:10
								...
								columns :[
											...
											plugins : [rowediting,Grid.Plugins.RowNumber,Grid.Plugins.AutoFit,Grid.Plugins.RadioSelection],
											// 底部工具栏
								bbar : {
												// pagingBar:表明包含分页栏
												pagingBar : true
								...
								textGetter: function(item){ //返回选中的id
										   return item.id;//Picker中用来返回值
										  },
							...
							picker = new Picker.ListPicker({
						        ...
						        children : [dictgrid] //配置picker内的列表
						      }).render();
							...
							function delFunction() {
								...
								BUI.Message.Confirm(
												'确认删除么？',function() {
													...
													$.post("/ms/flow/dict/deleteBatch",{ids : ids.toString()},
							...
							function deldFunction() {
								...
								BUI.Message.Confirm(
												'确认删除么？',function() {
													...
													$.post("/ms/flow/dict/casdelete",{ids : ids.toString()},
															function(data,status) {
