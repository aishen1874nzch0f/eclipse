...
public class TaskController extends SnakerController {
	...
	public void toDoList() {
		...
		String[] assignees = new String[list.size()];
		list.toArray(assignees);
		Page<WorkItem> page = new Page<WorkItem>(5);
		page.setPageNo(getParaToInt("pageIndex", 1));
		List<WorkItem> majorWorks =engine.query().getWorkItems(page, 
				new QueryFilter().setOperators(assignees).setTaskType(TaskType.Major.ordinal()));
		Map<String, Object> map=MapUtil.toMap("rows", majorWorks);
		map.put("results",page.getPageSize());
...
//		render("userTask.jsp");
	...
	public void aiDoList() {
		...
		Page<WorkItem> page = new Page<WorkItem>(5);
		page.setPageNo(getParaToInt("pageIndex", 1));
		List<WorkItem> aidantWorks = engine.query().getWorkItems(page, 
				new QueryFilter().setOperators(assignees).setTaskType(TaskType.Aidant.ordinal()));
		Map<String, Object> map=MapUtil.toMap("rows",aidantWorks);
		map.put("results",page.getPageSize());
		map.put("hasError", false);		
		renderJson(map);
//		setAttr("page", page);
//		render("userTask.jsp");
	 ...
	 * 根据当前用户查询抄送任务列表
	 */
	public void ccDoList() {
		List<String> list = ShiroUtils.getGroups();
		list.add(ShiroUtils.getUsername());
		log.info(list.toString());
		String[] assignees = new String[list.size()];
		list.toArray(assignees);
		Page<HistoryOrder> page = new Page<HistoryOrder>(5);
		page.setPageNo(getParaToInt("pageIndex", 1));
		List<HistoryOrder> ccWorks=engine.query().getCCWorks(page, 
				new QueryFilter().setOperators(assignees).setState(1));
		Map<String, Object> map=MapUtil.toMap("rows", ccWorks);
		map.put("results",page.getPageSize());
		map.put("hasError", false);		
		renderJson(map);
//		setAttr("page", page);
...
//		render("activeTaskMore.jsp");
		Map<String, Object> map=MapUtil.toMap("rows", page.getResult());
		map.put("results",page.getPageSize());
...
//		render("activeCCMore.jsp");
	...
	public void reject() {
		...
		redirect("/ms/flow/task/active" + error);
	}
